...
$professorQuery = "SELECT * FROM students_professors WHERE user_ID = '{$userID['User_ID']}' AND Is_Professor = 1";
...
$managerQuery = "SELECT * FROM school_unit_manager WHERE User_ID = '{$userID['User_ID']}'";
$managerResult = mysqli_query($conn, $managerQuery);
$isManager = mysqli_num_rows($managerResult) > 0;

// Check if the user is a unit manager
$ADMINQuery = "SELECT * FROM administrator WHERE User_ID = '{$userID['User_ID']}'";
...
$SPuserDetailsQuery = "SELECT users.first_name, users.last_name, school_unit.School_Name
                    ...
                    WHERE users.User_ID = '{$userID['User_ID']}'";
