...
if (isset($_GET['School_ID'])) {
  ...
  while ($book = mysqli_fetch_assoc($booksResult)) {
    ...
    $bookID = $book['Book_ID'];
    // Retrieve available copies from school_book table
  $copiesQuery = "SELECT Available_Copies FROM school_book WHERE Book_ID = ? AND School_ID = ?";
  $copiesStmt = mysqli_prepare($conn, $copiesQuery);
  mysqli_stmt_bind_param($copiesStmt, 'ii', $bookID, $schoolID);
  mysqli_stmt_execute($copiesStmt);
  $copiesResult = mysqli_stmt_get_result($copiesStmt);

  if ($copiesRow = mysqli_fetch_assoc($copiesResult)) {
    $availableCopies = $copiesRow['Available_Copies'];
  } else {
    $availableCopies = 0; // Default value if no copies found
  }
    ...
    <div class="book">
      ...
      <div class="book-details">
        ...
        <div class="book-info">
          ...
          <p><strong>Available Copies:</strong> <?php echo $availableCopies; ?></p> <!-- Display available copies -->
        </div>
